...
**Styling**: "Metamorphosis" theme — a midnight coffee house drawn by M.C. Escher. Ink-and-paper woodcut aesthetic: espresso grounds, bone-paper text, roasted-amber accent. CSS variables in `index.css` (`--font-display`: Fraunces, `--font-ui`: Hanken Grotesk, `--font-content`: Literata, `--font-code`: IBM Plex Mono). Primary accent roasted amber (#d4913c); stage pigments are lithographic: stage 1 verdigris (#7dab9c), stage 2 terracotta (#c96f4a), stage 3 / chairman crema gold (#e8b558); danger is fired brick (#cd6a4a), success olive (#a3b56d). Escher machinery lives in `index.css`: `--tess-cubes` / `--tess-cubes-line` (exact rhombille "tumbling blocks" SVG tilings, tile 48×72), `.tess-veil`, `.plate-frame`, `.escher-rule`; `EscherBand.jsx` is the square→cube→hexagon→bean→bird metamorphosis ornament (login + final essay). Buttons follow the "ink stamp" pattern (amber bg, espresso text, 3px radius); cards use 6px radii with double-ruled woodcut frames on key surfaces. NEVER reintroduce cool blues/slates (#3b82f6, #0f172a, #1e293b families) — everything stays in the warm ink palette.
